...
class NetworkEquipment: 
    def __init__(self, root):
        ...
        self.create_widgets()
        self.obtener_items_rack(self.obtener_token_sesion(), "57")
        #self.obtener_todos_los_racks_glpi(self.obtener_token_sesion())
    ...
    # --- Metodos para registro en los racks ---

    def obtener_todos_los_racks_glpi(self, session_token):
        """
        Obtiene la lista de racks desde GLPI mediante la API REST.
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return racks  # Retorna la lista de racks como un diccionario
            else:
                print(f"Error al obtener racks: {response.status_code} - {response.text}")
                return None
        except requests.exceptions.RequestException as e:
            print(f"Error de conexión con la API de GLPI: {e}")
            return None


    def obtener_items_rack(self, session_token, id_rack):
        """
        Obtiene los elementos asociados a un rack en GLPI.
        """
        url = f"{GLPI_URL}/search/item_racks"
        headers = {
            "Content-Type": "application/json",
            "Session-Token": session_token,
            "App-Token": APP_TOKEN
        ...
        params = {
            "criteria[0][field]": "racks_id",
            "criteria[0][searchtype]": "equals",
            "criteria[0][value]": id_rack,
            "range": "0-50"  # Limita la cantidad de resultados
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Error al obtener ítems del rack {id_rack}: {response.status_code} - {response.text}")
                return []
        except requests.exceptions.RequestException as e:
            print(f"Error de conexión con GLPI: {e}")
            return []
